...
module.exports.run = async (bot, message, args) => {
  ...
  if (member.id === message.author.id) return message.channel.send(lewdembedmm).catch(e =>  message.channel.send(e)).catch(O_o => {})
    if(!reason) return message.channel.send("You need to specify the amount you trying to give !");
    if (reason.includes("-")) {
        message.channel.send(`Invalid amount !`);
        return;
      }
    if(reason > currencylogs[message.author.id].coins) return message.channel.send(`You don't have enough money to share !`);
    ...
    if(reason < 1) return message.channel.send(`You can't pay less than $1`);
    ...
    let amount2 = parseInt(currencylogs[member.user.id].coins, 10);
    let amount3 = parseInt(currencylogs[message.author.id].coins, 10);
    currencylogs[message.author.id] = {
      coins: amount3 - amount,
  ...
  fs.writeFile("./storage/currencylogs.json", JSON.stringify(currencylogs), (err) => {
    if(err) console.log(err)})
            ...
            let embed = new Discord.MessageEmbed()
                ...
                .setColor("RANDOM")
                .setDescription(`\nYou have given $${amount} to ${member.displayName}'s balance`)
                .setFooter(`They have $${currencylogs[member.user.id].coins} now and you have $${currencylogs[message.author.id].coins} left`)
