...
module.exports.run = async (bot, message, args) => {
  ...
  let rainbow = message.guild.roles.cache.find(r => r.name === "𝘚𝘶𝘱𝘱𝘰𝘳𝘵𝘦𝘳 🏆");
  if (!rainbow)return;
...
const missingArgsEmbed = new discord.MessageEmbed() // Creates the embed thats sent if the command isnt run right
        ...
        .setDescription('Usage: supporter [User]')
  ...
  if (person.roles.cache.has(rainbow.id)) {
    ...
    const lewdembedm = new discord.MessageEmbed()
      .setColor(`#000000`)
      .setDescription(`✔︎ | **${message.author.username}** has removed **${person.displayName}**'s 𝘚𝘶𝘱𝘱𝘰𝘳𝘵𝘦𝘳 :military_medal: role !`);
  ...
  const lewdembedm = new discord.MessageEmbed()
    .setColor(`#000000`)
    .setDescription(
      `✔︎ | **${person.displayName}** has been given their 𝘚𝘶𝘱𝘱𝘰𝘳𝘵𝘦𝘳 :military_medal: role !`
    );
