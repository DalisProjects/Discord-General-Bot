...
module.exports.run = async (bot, message, args) => {
  ...
  //if(premiumserver[message.guild.id].premiumserver == 0)return message.channel.send(embedps);
    ...
    if (timeout - (Date.now() - currencylogs[message.author.id].hourly) > 0) {
      let time = ms(timeout - (Date.now() - currencylogs[message.author.id].hourly));
  
      let timeEmbed = new Discord.MessageEmbed()
      .setColor("#FFFFFF")
      .setDescription(`You are still on cooldown\n\nYou can get your hourly paycheck again in ${time.minutes}m ${time.seconds}s `);
      message.channel.send(timeEmbed)
      return
    }

      let randomm = Math.floor(Math.random() * 1000) + 2000;
    ...
    let base = amount
      ...
      let embed = new Discord.MessageEmbed()
          ...
          .setColor("RANDOM")
          .setDescription(`\nClaimed hourly paycheck, added $${amount} to your balance`)
