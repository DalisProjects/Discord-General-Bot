...
module.exports.run = async (bot, message, args) => {
  ...
  //if(premiumserver[message.guild.id].premiumserver == 0)return message.channel.send(embedps);
      ...
      if(!reason) return message.channel.send("You need to specify the amount that you want to withdraw !");
      if (reason.includes("-")) {
      ...
      if(reason > currencylogs[message.author.id].bank) return message.channel.send(`You don't have enough money to withdraw !`);
      if (isNaN(reason) && reason !== "all") return message.channel.send(`The specified amount isn't valid`); 
      if(reason === "all") reason = currencylogs[message.author.id].bank;
      if(reason < 1) return message.channel.send(`You cannot withdraw $0 from your bank!`);
    ...
    let base = amount
      ...
      let embed = new Discord.MessageEmbed()
          ...
          .setColor("RANDOM")
          .setDescription(`\n$${reason} withdrawn, $${currencylogs[message.author.id].bank} left in your bank`)
          .setFooter(`Your have  $${currencylogs[message.author.id].coins} in your wallet now`);
